...
const Home = () => {
  ...
  const handleGoClick = () => {
    if ( selectedCrop && selectedPercentType) {
      router.push({
        pathname: `addMapDates/add${selectedCrop.slug}MapDatePlebian`,
        query: {
          // state: selectedState.value, removed state cause not non admins dont alternate betweens tates
