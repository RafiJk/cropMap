...
const Updater = () => {
  ...
  useEffect(() => {
    ...
    const makeNewDoc = () => { //access collection and iterate through and create it in form
      const newMapDocRef = doc(collection(db, "SMap")); 
    ...
    const fetchCountyData = async () => {
      ...
      const cmq = query(SMapCollection, orderBy('date', 'desc'), limit(1));
      const querySnapshot = await getDocs(cmq);
      const mostRecentDocumentName = querySnapshot.docs[0].id.toString();
      const documentRef = doc(SMapCollection, mostRecentDocumentName);
      try {
