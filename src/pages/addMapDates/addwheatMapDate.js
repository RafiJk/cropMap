...
const AddHarvest = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const WheatMapQuery = query(WheatMapCollection, orderBy("date", "desc"), limit(1));
      const stepone = await mostRecentDocument(WheatMapQuery);
      let steptwo;
      if (stepone === true) {
        steptwo = NewDocMaker();
      } else {
    ...
    const fetchCountyData = async () => {
      ...
      if (steptwo === true) {
        ...
        const cmq = query(WheatMapCollection, where("id", "==", mostRecentName));
  ...
  const getDocumentName = async () => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        console.log('No documents found in WheatMap collection.');
      }
    } catch (error) {
