...
import Legend from "../cropMap/legend"; // Adjust the path
...
const MapChart = () => {
  ...
  const auth = useAuth(); // Get the authentication state from your authentication module

  useEffect(() => {
    if (auth.user) {
      const userState = auth.user.state; // Assuming state information is stored in the user's data

      // Use userState to filter the stateGeographyData and fetch the relevant map
      const selectedStateData = stateGeographyData.find(stateData => stateData.stateCode === userState);

      // Fetch the map using selectedStateData.url and proceed similarly to the rest of the component logic
      // ...
    }
  }, [auth.user]);
